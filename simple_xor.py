...
with tf.name_scope("layer1"):
    W1 = tf.Variable(tf.random_normal(shape=[2, 10]), name="weights")
    b1 = tf.Variable(tf.random_normal(shape=[10]), name="bias")
    h1 = tf.nn.sigmoid(tf.matmul(x, W1) + b1)

with tf.name_scope("layer2"):
    W2 = tf.Variable(tf.random_normal(shape=[10, 2]), name="weights")
